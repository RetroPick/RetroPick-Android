...
const KEYWORD_MAP: Record<MarketCategoryEnum, string[]> = {
  ...
  Sports: [
    ...
    'rampage', 'first blood', 'exact score', 'completed match', 'vs'
  ],
  Finance: [
  ...
  AI: [
    'openai', 'chatgpt', 'gpt-4', 'gpt-5', 'gpt', 'gemini', 'anthropic', 'claude',
  ...
  Economics: [
    ...
    'interest rate cut', 'interest rate hike', 'interest rate'
  ],
  Climate: [
  ...
  try {
    ...
    const mapped = rawMarkets.map((m: any, idx: number): Market | null => {
      ...
      const isBanned = BANNED_KEYWORDS.some((kw) =>
        qLower.includes(kw) ||
        originalCat.includes(kw) ||
        tagSlugs.some((s: string) => s.includes(kw))
      )
      ...
      let marketType: Market['marketType'] = 'UP_OR_DOWN'
